...
get_sources(){
    ...
    # percona-packaging/ already lives inside the source repo (PSMDB/PBM
    # convention), so the clone above brings it in. Sanity-check that the
    # branch we picked actually contains the packaging — otherwise the
    # downstream RPM/DEB stages will fail with a confusing tar error.
    if [ ! -d ${PRODUCT}-${VERSION}/percona-packaging ]; then
        echo "ERROR: ${REPO} @ ${BRANCH} does not contain percona-packaging/."
        echo "Make sure you are pointing at the Percona fork (with packaging),"
        echo "not the upstream mongodb/mongot."
        exit 1
    fi
